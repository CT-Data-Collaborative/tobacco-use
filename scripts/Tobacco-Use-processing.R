...
##################################################################
#
# Processing Script for Tobacco Use
...
#Bring together all legacy Tobacco Tables
...
for (i in 1:length(legacy_table_dfs)) {
  ...
  descriptions <- c("Tobacco Product Use in the Past Month", 
...
new_table_dfs <- c("2012-2014 Substate SAE Table 12", 
                   "2012-2014 Substate SAE Table 13", 
                   "2014-2016 Substate SAE Table 7",  
                   "2014-2016 Substate SAE Table 8")
...
test$`Tobacco Use` <- NA
test$`Tobacco Use`[grep("Cigarette", test$`Table Description`)] <- "Cigarette Use in the Past Month"
test$`Tobacco Use`[grep("Tobacco", test$`Table Description`)] <- "Tobacco Product Use in the Past Month"
...
#assign variable
all_data_long$Variable[grep("Estimate", all_data_long$Variable)] <- "Tobacco Use"
...
all_data_final <- all_data_long %>% 
  select(`Substate Region`, Year, `Age Range`, `Tobacco Use`, `Measure Type`, Variable, Value) %>% 
  rename(Region = `Substate Region`) %>% 
  arrange(Region, Year, `Age Range`, `Tobacco Use`, Variable)
...
write.table(
  all_data_final,
  file.path(getwd(), "data", "tobacco-use-2016.csv"),
  sep = ",",
  na = "-9999",
  row.names = F
)
